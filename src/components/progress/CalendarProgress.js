...
const CalendarProgress = () => {
  ...
  const progressData = [
    {
      title: "Accepted",
      value: (calendar?.total ?? 0) - (calendar?.upcoming ?? 0) - (calendar?.recurrent ?? 0),
      percentage: ((calendar?.total - (calendar?.upcoming ?? 0) - (calendar?.recurrent ?? 0)) / calendar?.total) * 100 || 0,
  ...
  console.log("calender Data: ", JSON.stringify(calendar, null, 2));
    ...
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        <View style={{ flexDirection: "row", gap: 10, alignItems: "center" }}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate("MyCalenderStack", { screen: "MyCalender" })}>
            <ArrowTopRight />
          </TouchableOpacity>
        </View>

...
const DayToDayChart = () => {
  ...
  const day =
    dayToday?.map((result) =>
      result.label.startsWith("Week") ? `W ${result.label.split(" ")[1]}` : moment(result?.label, "MMMM YYYY").format("MMM YY")
    ) || [];

  const CustomBar = ({ x, y, bandwidth, data }) =>
    data.map((value, index) => (
      <Rect key={index} x={x(index)} y={y(value)} rx={5} ry={5} width={bandwidth} height={y(0) - y(value)} fill={Colors.Primary} />
    ));
  ...
  return (
    <View>
      <View style={styles.titleContainer}>
        ...
        <CustomDropDownTwo flex={0.6} data={option} state={timeFrame} setState={setTimeFrame} />
      </View>
      <View>
        <BarChart

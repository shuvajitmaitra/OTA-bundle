  ...
  return (
    <View style={styles.eventTopicContainer}>
      ...
      <View style={{ width: "70%" }}>
          ...
          return (
            <View key={singleItem?._id} style={styles.viewWidth}>
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  user._id === singleItem?.createdBy?._id
                    ? (getEventDetails(singleItem?._id), getNotificationData(singleItem?._id), toggleUpdateModal())
                ...
                style={[
                  styles.eventTitleAndDateContainer,
                  {
                    backgroundColor:
                      ...
                      (singleItem?.eventType === "mockInterview" && "#f59f9f") ||
                      (singleItem?.eventType === "orientation" && "#379793") ||
                      (singleItem?.eventType === "technicalInterview" && "#f8a579") ||
                      (singleItem?.eventType === "behavioralInterview" && "#0091b9") ||
                      (singleItem?.eventType === "reviewMeeting" && "#7ccc84") ||
              ...
              >
                ...
                >
                  {moment(singleItem?.start).format("h:mm A") + " - " + moment(singleItem?.end).format("h:mm A")}
                </Text>
              </TouchableOpacity>
        ...
        {updateModalVisible && <UpdateEventModal modalVisible={updateModalVisible} setModalVisible={setUpdateModalVisible} />}
      </View>
    </View>
  );
};

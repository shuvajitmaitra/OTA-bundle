    ...
    <View style={styles.container}>
      ...
      {dataToDisplay?.map((benefit) => (
        <View key={benefit._id} style={styles.pointContainer}>
          <Feather style={{ marginTop: 3 }} name="check-circle" size={24} color={Colors.Primary} />
          <Text style={styles.benefitsPointText}>{benefit.title}</Text>
        </View>
      ))}
      <TouchableOpacity onPress={() => setShowMore(!showMore)} style={styles.showMoreButton}>
        <Text style={styles.showMoreButtonText}>{!showMore ? "See More" : "See Less"}</Text>
      </TouchableOpacity>
    </View>
  );
};

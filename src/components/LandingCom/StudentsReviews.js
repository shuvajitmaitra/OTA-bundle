...
const StudentsReviews = ({ courseId }) => {
  ...
  useEffect(() => {
    if (activeCategories && activeCategories.length > 0) {
      const defaultCategory = activeCategories.find((category) => category.isDefault === true);
      const otherCategories = activeCategories.filter((category) => category.isDefault === false);
      if (defaultCategory) {
  ...
  useEffect(() => {
    const defaultCategory = reviewsData?.categories?.find((item) => item?.isDefault);
  ...
  return (
    <View style={styles.container}>
              ...
              (tab, index) =>
                tab?.isActive && (
                  <TouchableOpacity
                    ...
                    style={[styles.tabButton, status === tab?.name && styles.tabActiveButton]}
                  >
                    <Text style={[styles.tabText, status === tab?.name ? styles.activeTabText : styles.inactiveTabText]}>{tab?.name}</Text>
                  </TouchableOpacity>
                )
            )}
          </View>
      ...
      <ReviewsBody courseId={courseId} categories={categories} category={status} type={type} setLimit={setLimit} limit={limit} />
    </View>
  );
};

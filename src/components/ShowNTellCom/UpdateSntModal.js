...
import { formattingDate, showAlertModal } from "../../utility/commonFunction";
import RedCrossIcon from "../../assets/Icons/RedCrossIcon";
import RequireFieldStar from "../../constants/RequireFieldStar";

export default function UpdateSntModal({ setIsUpdateSntModalVisible, isSntUpdateModalVisible, toggleUpdateSntModal, item }) {
  ...
  const [selectedCustomTime, setSelectedCustomTime] = useState(moment(item?.date).format("hh:mm A")); // Store the selected time
  ...
  }, [item?._id, title, agenda, selectedUsers, selectedDate, selectedCustomTime]);

  useEffect(() => {
    axiosInstance
      .post("user/filter", { query: "" })
      .then((res) => {
        setUsers(res.data.users.filter((item) => !selectedUsers.map((i) => i._id)?.includes(item._id)));
      ...
      {updating ? (
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalTop}>
            <ModalBackAndCrossButton toggleModal={toggleUpdateSntModal} />
          </View>
          <ScrollView>
            <Text style={styles.modalHeading}>Update Show N Tell</Text>
            <Text style={styles.modalSubHeading}>Please fill out the form to update a Show and Tell.</Text>
            ...
            <TouchableOpacity onPress={() => showCalendar()}>
              <CalenderIcon size={18} color={Colors.BodyText} />
                ...
                >
                  {selectedDate && selectedCustomTime ? `${formattingDate(selectedDate)} ${selectedCustomTime}` : "Pick a date"}
          ...
          />
            ...
            <View style={styles.btnArea}>
              <MyButton
                onPress={() => setIsUpdateSntModalVisible(false)}
                title={"Cancel"}
                bg={"rgba(39, 172, 31, 0.10)"}
                colour={Colors.Primary}
              />
              <MyButton onPress={handleUpdateSnt} title={"Update"} bg={Colors.Primary} colour={Colors.PureWhite} />
            </View>
          </ScrollView>
        </View>
      )}

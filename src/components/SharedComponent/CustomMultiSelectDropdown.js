...
export const CustomMultiSelectDropDown = ({ options, initialSelections = [], setState }) => {
  ...
  return (
    <TouchableWithoutFeedback style={{ backgroundColor: "red", padding: 100 }} onPress={handleOutsidePress}>
      <View style={{ flex: 1 }}>
        <TouchableOpacity
          style={[
            styles.inputField,
        ...
        >
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 5, flex: 0.9 }}>
            {selectedItems.map((item, index) => (
              <TouchableOpacity onPress={() => toggleSelection(item)} style={styles.selectedItemsContainer} key={index}>
                <Text
                  style={{
          ...
          <View style={styles.dropdownOptions}>
            {options.map((item, index) => (
              ...
              >
                ...
                >
                  <Text style={[styles.Text, selectedItems.some((i) => i.type === item.type) && styles.selectedText]}>{item.type}</Text>
                  <TouchableOpacity
                    style={[
                    ...
                    onPress={() => toggleSelection(item)}
                  >
                    <CrossCircle size={15} color={selectedItems.some((i) => i.type === item.type) ? Colors.Red : null} />
                  </TouchableOpacity>
                </View>
                <View
                  style={{
